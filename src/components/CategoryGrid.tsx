...
/**
 * Images are matched exactly to the real category names from simba_products.json.
 * Every key here is a real category name from the dataset.
 */
const CATEGORY_META: Record<string, { image: string; bg: string }> = {
  // Fresh food & groceries
  'Groceries': {
    image: 'https://images.unsplash.com/photo-1543168256-418811576931?w=200&q=80',
  ...
  // Wine, whisky, beer, spirits
  'Alcoholic Beverages & Spirits': {
    image: 'https://images.unsplash.com/photo-1569529465841-dfecdab7503b?w=200&q=80',
    bg: 'bg-red-50 dark:bg-red-900/20',
  },
  // Mops, sponges, detergents, toilet cleaners
  'Cleaning & Sanitary': {
    image: 'https://images.unsplash.com/photo-1563453392212-326f5e854473?w=200&q=80',
    bg: 'bg-cyan-50 dark:bg-cyan-900/20',
  },
  // Water bottles, canisters, storage containers
  'Kitchen Storage': {
    image: 'https://images.unsplash.com/photo-1584568694244-14fbdf83bd30?w=200&q=80',
    bg: 'bg-lime-50 dark:bg-lime-900/20',
  },
  // Sausages, beef, meat products (mislabelled as Pet Care in dataset)
  'Pet Care': {
...
export default function CategoryGrid({ categories, onSelect }: Props) {
  const { selectedCategory } = useSimbaStore();
      ...
      {categories.map((cat, i) => {
        ...
        return (
          ...
          >
            {/* Category image */}
            <div className={clsx(
              'w-14 h-14 rounded-xl overflow-hidden relative flex-shrink-0',
              meta.bg
            )}>
              <Image
                src={meta.image}
