...
// ── Instant demo credentials — no DB needed ──────────────────────────────────
// These match what the Express backend seeds. Works even if DB/Render is cold.
const DEMO_STAFF: Record<string, { password: string; name: string; branchId: string; branchName: string; role: 'manager' | 'staff' }> = {};
const BRANCHES = [
...
export async function POST(req: NextRequest) {
  ...
  // ── 1. Instant path: check demo/seeded credentials without DB ────────────
  ...
  if (demo && password === demo.password) {
    ...
    // Fire-and-forget: sync to DB in background (don't await)
  ...
  // ── 2. DB path: custom credentials added via DB ──────────────────────────
  try {
    const pool = getPool();
