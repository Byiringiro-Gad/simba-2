...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    try {
      if (id > 900000) {
        await conn.execute(
          `UPDATE product_additions SET name=?, price=?, category=?, unit=?, image=?, in_stock=?, stock_count=?, description=?, updated_at=NOW() WHERE id=?`,
        ...
        await conn.execute(
          ...
          [id, name ?? base?.name ?? '', Math.round(price ?? base?.price ?? 0), category ?? base?.category ?? '',
           unit ?? base?.unit ?? 'Pcs', image ?? base?.image ?? '',
           inStock !== undefined ? (inStock ? true : false) : (base?.inStock ?? true),
           stockCount ?? base?.stockCount ?? 100, description ?? null]
        );
      }
